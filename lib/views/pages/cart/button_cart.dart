...
import '../../../core/dialogs/dialogs_cart/dialogs_cart_delete_and_check_available.dart';
import '../../../core/dialogs/dialogs_cart/dialogs_no_tawseel_free.dart';
  ...
  async {
    ...
    try {
      ...
      if (hasWarningMessages) {
        ...
        printLog("Stopping checkout due to warning messages - user alerted with vibration");
        return; // Stop the checkout process
      }
      
      if (cartController.notAvailabilityItems.isEmpty &&
          cartController.availabilityItems.length == cartController.cartItems.length) {
        
        // All items are available, check free shipping
        var freeShipValue = await FirebaseRemoteConfigClass().fetchFreeShip();
        
        if (int.parse(freeShipValue.toString()) == 0  || (double.parse(cartController.totalItemsPrice.toString()) >
          ...
          await pointsController.changeTotal(double.parse(cartController.totalItemsPrice.toString()));
          ...
          NavigatorApp.push(CheckoutFirstScreen(
            ...
            double remainder = double.parse(freeShipValue.toString()) - 
 ...
 // Close waiting dialog
        await scrollController.scrollTo(
          index: cartController.availabilityWithFalseItems
              .indexOf(cartController.availabilityWithFalseItems.first),
          duration: Duration(milliseconds: 300),
          curve: Curves.easeInOut,
        );
      }
  ...
  Widget build(BuildContext context) {
    return Consumer4<CartController, FetchController, PointsController, PageMainScreenController>(
      builder: (context, cartController, fetchController, pointsController, pageMainScreenController, child) {
        return (cartController.cartItems.isNotEmpty && cartController.haveButton)
            ? ButtonDone(
