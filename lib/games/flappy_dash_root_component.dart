...
import 'package:fawri_app_refactor/games/pipe_pair.dart';
...
class FlappyDashRootComponent extends Component
  ...
  Future<void> onLoad() async {
    ...
    _generatePipeAnDCoins(
        count: 5, fromX: 400.w); // إنشاء الأنابيب والعملات عند بدء اللعبة
  ...
  void _generatePipeAnDCoins({required int count, double fromX = 0.0}) {
    for (int i = 0; i <= count; i++) {
      ...
      add(lastPipe = PipePair(
          position: Vector2(
              (fromX + ((i * (basicDistance)) + 100.w)), y))); // إضافة أنبوب
    }
  }
  ...
  void update(double dt) {
    if (dashBird.x >= lastPipe.x) {
      _generatePipeAnDCoins(
          count: 5, fromX: basicDistance); // توليد أنابيب وعملات جديدة
  ...
  void _removeOldOnes() {
    ...
    final shouldBeRemovedPipes =
        max(pipes.length - 5, 0); // إبقاء آخر 5 أنابيب فقط
