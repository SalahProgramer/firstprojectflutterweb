import 'package:fawri_app_refactor/salah/models/items/variants_model.dart';
...
class Item {
  ...
  Item.fromJsonProductSpecificId(Map<dynamic, dynamic> json)
    : id = json["id"] ?? "",
      ...
      variants = (json["variants"].isEmpty)
          ...
          : Variants.fromJsonList(json["variants"], json["max_purchase"] ?? 0),
      status = json["status"],
  ...
  // Adjusting fromJsonProduct to use `seen` and initialize required fields
  Item.fromJsonProductBestOrFlash(
  ...
  Item.fromJsonSimilarProduct(Map<dynamic, dynamic> json)
    : id = json["id"] ?? "",
      title = json["title"] ?? "",
      thumbnail = json["thumbnail"] ?? "",
      newPrice = (json["price"] != null)
  ...
  static List<Item> fromJsonListSimilarProducts(List<dynamic> jsonList) {
    return jsonList.map((json) {
      ...
      return Item.fromJsonSimilarProduct(jsonMap);
    }).toList();
  }
  ...
  Map<String, dynamic> toJsonProduct() => {
    ...
    "medium_vendor_images_links": mediumVendorImagesLinks
        ?.map((x) => x)
        .toList(),
    "shop_id": shopId,
    ...
    "id": id,
    "thumbnail": thumbnail,
    "google_cloud_images_links": googleCloudImagesLinks?.map((x) => x).toList(),
