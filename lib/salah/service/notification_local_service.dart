...
import 'package:fawri_app_refactor/salah/views/pages/notifications/notifications_page.dart';
...
class NotificationService {
  ...
  static Future<void> onNotificationCreatedMethod(
    ...
    printLog(
      'onNotificationCreatedMethod (created notification)',
  ...
  static Future<void> onNotificationDisplayedMethod(
    ...
    printLog('onNotificationDisplayedMethod (displayed)');

    // CRITICAL: Check if notifications are disabled and cancel immediately
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      bool appEnabled = prefs.getBool('app_notifications_enabled') ?? true;

      printLog(
        'Awesome Notification displayed - app_notifications_enabled: $appEnabled, ID: ${receivedNotification.id}',
      );

      if (!appEnabled && receivedNotification.id != null) {
        // Cancel this notification immediately as a last resort
        await AwesomeNotifications().cancel(receivedNotification.id!);
        printLog(
          '🚫 Awesome Notification ${receivedNotification.id} cancelled - app notifications disabled',
        );
      }
    } catch (e) {
      printLog('Error checking/cancelling notification: $e');
    }
  }
  ...
  static Future<void> onDismissActionReceivedMethod(
    ...
    printLog(
      'onDismissActionReceivedMethod   (recived/dismiss notification)',
    ); // تسجيل في الـ Console
  }
    ...
    try {
      ...
      await AwesomeNotifications().createNotification(
        content: NotificationContent(
          ...
          payload: const {'navigate': 'true', 'destination': 'cart'},
        ),
