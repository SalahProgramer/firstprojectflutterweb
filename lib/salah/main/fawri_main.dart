...
class _FawriState extends State<Fawri> {
  ...
  Future<void> initializeApp() async {
    try {
      ...
      final ProductItemController productItemController = context
          .read<ProductItemController>();
      final HiveCollection hiveCollection = context.read<HiveCollection>();
      final FavouriteController favouriteController = context
          .read<FavouriteController>();
      final NotificationController notificationController = context
          .read<NotificationController>();
      final ShowcaseController showcaseController = context
  ...
  Future<void> initializeDataFromDatabase({
    ...
    try {
      await Future.wait<void>([
        ...
        notificationController.initializeNotificationStatus().catchError((
          e,
          stack,
        ) {
          SentryService.captureError(
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (context) => GamesCubit(getIt.get<AudioHelper>())),
        BlocProvider(
          create: (context) {
      ...
      child: BlocBuilder<InternetCubit, InternetState>(
        builder: (context, state) {
          return ScreenUtilInit(
            ...
            child: ShowCaseWidget(builder: (context) => Splash()),
          );
        },
      ),
    );
