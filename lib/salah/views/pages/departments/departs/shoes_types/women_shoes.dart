...
class _WomenShoesState extends State<WomenShoes> with DepartmentViewMixin {
  ...
  Future<void> _navigateWithSizes(String sizes) async {
    ...
    final category = CategoryModel.fromJsonListCategory(womenShoes);
    await controller.setSubCategoryDepartments(womenShoes, false);
  ...
  Widget build(BuildContext context) {
    ...
    return WidgetEachDepartment(
      ...
      isLoadingType: controller.isLoadingWomenShoes,
      methodChangeLoading: controller.changeLoadingWomenShoes,
      backgroundImage: "",
      title: "أحذية ستاتية",
      check: controller.haveCheckWomenShoes,
      onPressedSure: () async {
        // MVC: View uses helper to extract data, then navigates
        final sizes = DepartmentViewHelpers.extractSelectedSizes(
          controller.isLoadingWomenShoes,
        );
