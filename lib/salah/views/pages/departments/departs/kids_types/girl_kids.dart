...
class _GirlKidsState extends State<GirlKids> with DepartmentViewMixin {
  ...
  Future<void> _navigateWithSizes(String sizes) async {
    ...
    final category = CategoryModel.fromJsonListCategory(girls);
    await controller.setSubCategoryDepartments(girls, false);
  ...
  Widget build(BuildContext context) {
    ...
    return WidgetEachDepartment(
      ...
      isLoadingType: controller.isLoadingGirlKids,
      methodChangeLoading: controller.changeLoadingGirlKids,
      backgroundImage: "",
      title: 'قسم البنات',
      check: controller.haveCheckGirlKids,
      onPressedSure: () async {
        // MVC: View uses helper to extract data, then navigates
        final sizes = DepartmentViewHelpers.extractSelectedSizes(
          controller.isLoadingGirlKids,
        );
