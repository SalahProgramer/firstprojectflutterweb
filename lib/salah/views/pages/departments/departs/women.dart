...
class Women extends StatefulWidget {
  ...
  @override
  State<Women> createState() => _PerfumeHomeState();
}

class _PerfumeHomeState extends State<Women> {
  @override
  Widget build(BuildContext context) {
    ...
    return WidgetEachDepartment(
      ...
      listStyle2: womenSizesList.entries
          .map((entry) => {"sizeText": entry.key, "sizeSubtitle": entry.value})
          .toList(),
      isLoadingType: departmentsController.isLoadingWomenClothesTypes,
      methodChangeLoading: departmentsController.changeLoadingWomenClothes,
      backgroundImage: "",
      title: "ملابس النسائية",
      check: departmentsController.haveCheckWomenClothes,
      onPressedSure: () async {
        await departmentsController.clearAll();
        var category = CategoryModel.fromJsonListCategory(women);

        await departmentsController.setSubCategoryDepartments(women, false);
        await departmentsController
            .setSubCategorySpecificFirstMulti(category[0]);

        String? sizes =
            departmentsController.isLoadingWomenClothesTypes?.entries
