...
class _SpecificProductViewState extends State<SpecificProductView> {
  ...
  final GlobalKey<SliderImagesViewState> sliderKey = GlobalKey<SliderImagesViewState>();

  /// Normalize and clean the incoming video URL similar to reels controller
  String _normalizeVideoUrl(String url) {
    if (url.isEmpty) return url;

    String normalized = url.trim();

    // Remove duplicate / malformed protocol patterns
    while (true) {
      ...
      if (normalized.toLowerCase().startsWith('https:///')) {
        ...
        normalized = 'http://${normalized.substring(20)}';
      } else if (normalized.toLowerCase().startsWith(
        'https://https://https://',
      )) {
        normalized = 'https://${normalized.substring(23)}';
      }
  ...
  /// Returns a normalized product video URL (or null if not available)
  String? get _productVideoUrl {
    final raw = widget.item.videoUrl?.toString() ?? '';
    if (raw.isEmpty) return null;
    return _normalizeVideoUrl(raw);
  }
  ...
  void dispose() {
    timer?.cancel();
    sliderKey.currentState?.pauseVideo(); // Pause video on dispose
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        if (didPop) {
          // Pause video when popping
          sliderKey.currentState?.pauseVideo();
        ...
        bottomNavigationBar: ButtonDone(
          ...
          onPressed: (widget.item.variants?[0].size == "")
              ...
              : (productItemController.inCart[widget.item.id.toString()] ==
                    true)
              ? () async {
                  // Pause video before cart operation
                  sliderKey.currentState?.pauseVideo();
              ...
              child: Padding(
                ...
                child: Badge(
                  ...
                  child: IconSvg(
                    ...
                    // heightIcon: 40.h,
                    onPressed: () async {
                      // Pause video before navigating to cart
                      sliderKey.currentState?.pauseVideo();
