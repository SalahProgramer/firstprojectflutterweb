import 'package:fawri_app_refactor/salah/controllers/APIS/api_departments_controller.dart';
...
import '../views/pages/departments/departs/kids_types/infant_baby.dart';
...
import '../utilities/networks/connection_network.dart';
...
class DepartmentsController extends ChangeNotifier {
  ...
  bool haveCheckInfantBaby = false;
  int numSort = 2;
  ...
  Map<String, bool>? isLoadingInfantBaby = {
    for (var size in womenAndBabySizesList) size.trim().toString(): false,
  };
  ...
  ApiDepartmentsController api = NavigatorApp.context
      .read<ApiDepartmentsController>();
  ...
  Future<void> changeLoadingInfantBaby(String type, bool check) async {
    isLoadingInfantBaby?[type] = check;
    haveCheckInfantBaby =
        isLoadingInfantBaby?.values.any((value) => value) ?? false;

    notifyListeners();
  }

  Future<void> clearInfantBaby() async {
    isLoadingInfantBaby = {
      for (var size in womenAndBabySizesList) size.trim().toString(): false,
    };
    haveCheckInfantBaby = false;
    notifyListeners();
  }
  ...
  Future<void> setMultiSelectSubDepartments(List<CategoryModel> items) async {
    ...
    if (items.length == 1 && items[0].name.contains("جميع") == true) {
      ...
      if (items.last.name.contains("جميع")) {
        ...
        CategoryModel? itemWithJame3 = listSelectedMultiSubCategoryDepartment
            .firstWhere(
  ...
  Future<void> tapSelectCategory({required CategoryModel categoryModel}) async {
    if (categoryModel.id == 1.toString()) {
      NavigatorApp.push(Women(category: categoryModel));
    } else if (categoryModel.id == 2.toString()) {
      NavigatorApp.push(Men(category: categoryModel));
    } else if (categoryModel.id == 3.toString()) {
      NavigatorApp.push(KidsAll(category: categoryModel));
    } else if (categoryModel.id == 4.toString()) {
      NavigatorApp.push(WomenPlus(category: categoryModel));
