import 'package:fawri_app_refactor/salah/controllers/APIS/api_order_controlller.dart';
...
class OrderControllerSalah extends ChangeNotifier {
  ...
  ApiOrderController apiOrderController = NavigatorApp.context
      .read<ApiOrderController>();

  // .substring(0, 10);

  //------------------------------------------------------------------------

  List<OrderDetailModel> newestOrder = [OrderDetailModel(orderId: 0)];
  List<OrderDetailModel> newestOrderData = [];

  List<OrderDetailModel> oldestOrder = [OrderDetailModel(orderId: 0)];
  ...
  Future<void> clear() async {
    ...
    numberOrderNewest = 0;
    numberOrderOldest = 0;
    newestOrder = [OrderDetailModel(orderId: 0)];
    newestOrderData = [];

    oldestOrder = [OrderDetailModel(orderId: 0)];
    notifyListeners();
  }

  /// Clear all order data completely (used when deleting account)
  Future<void> clearAllOrders() async {
    ...
    newestOrder = [OrderDetailModel(orderId: -1)];
    newestOrderData = [];
    oldestOrder = [OrderDetailModel(orderId: -1)];
    
    checkPendingOrder = null;
    totalDiscountAfterDelete.clear();
    reason.clear();
  ...
  Future<void> getCheckPendingOrder({required String phone}) async {
    checkPendingOrder = await apiOrderController.apiCheckPendingOrder(
      phone: phone,
    );
    notifyListeners();
  }
