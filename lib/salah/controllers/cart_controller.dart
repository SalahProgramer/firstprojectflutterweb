...
class CartController extends ChangeNotifier {
  ...
  ApiCartController apiCartController = NavigatorApp.context
      .read<ApiCartController>();
  ...
  Future<void> checkProductAvailability(
    ...
    try {
      ...
      for (var item in response) {
        ...
        String availabilityStatus = item.availability
            .toString()
            .trim()
            .toLowerCase();
        printLog(
          "Item ${item.id}: $availabilityStatus, message: ${item.message}, price: ${item.price}",
        );
        ...
        if (availabilityStatus == "true" &&
          ...
          String newTotalPrice = totalPrice1(
          ...
          pricesUpdated = true;
          printLog(
            "Updated price for item ${item.id}: ${item.price} -> $newPriceString",
          );
        }
      ...
      printLog(
        "Availability check completed: ${availabilityItems.length} available, ${notAvailabilityItems.length} not available, ${availabilityWithFalseItems.length} with false status",
      );
      notifyListeners();
    } catch (e) {
  ...
  Future<void> initializeDefaultAvailability() async {
    try {
      ...
      printLog(
        "Initialized default availability for ${availability.length} cart items",
      );
      notifyListeners();
    } catch (e) {
  ...
  Future<void> calculateItemOfferTawseel(int remainder, int itemPrice) async {
    totalItemsPriceOfferTawseel =
        totalItemsPriceOfferTawseel - itemPrice.toInt();
    if (totalItemsPriceOfferTawseel < 0) {
      totalItemsPriceOfferTawseel = 0;
    }
    notifyListeners();
  ...
  Future<int> updateData({
    ...
    int response = await sqlDb.updateData(
      sql:
          "UPDATE `cart` SET  `quantity` = $quantity,`total_price` = '$totalPrice' WHERE `id` = '$id';",
