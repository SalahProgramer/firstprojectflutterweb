...
class ApiProductItemController extends ChangeNotifier {
  ...
  Future<List<Item>> apiItemData(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (page == 1) {
          Item firstItem = Item.fromJsonProduct(
            res["item"],
            item.newPrice,
            item.oldPrice,
            item.videoUrl,
        ...
        if (haveSimilar == true) {
          List<Item> similarItems = Item.fromJsonListSimilarProducts(
            res["similar_items"],
          );
  ...
  Future<List<Item>> apiItemDataBestOrFlash(
    Item item, {
    required String sizes,
    required int page,
  }) async {
        ...
        if ((res["items"].isEmpty.toString()) == "true") {
          ...
          if (data.length == 1) {
            ...
            List<Item> newItems = data
                .where(
                  (item) => item.id.toString().trim() != myId.toString().trim(),
                )
                .toList();
