...
class ProductItemController extends ChangeNotifier {
  ...
  Future<void> doIncrement(String id, int index, Item item) async {
    if (valueQuantityItems[id]! >= int.parse(basicQuantityItems[id]!)) {
      ...
      if (inCart[id] == true) {
        int quantity =
            await cartController.checkCartItemQuantity(id: "${id}00$index");
    ...
    if (data != []) {
      ...
      for (var item in List.from(allItems)) {
        ...
        if ((item.variants?.length == 1) &&
          ...
          if (inCart[id] == true) {
            valueQuantityItems[id] =
                await cartController.checkCartItemQuantity(id: "${id}000");
          } else {
            valueQuantityItems[id] = 1;
          }
          ...
          for (var i in (item.variants ?? [])) {
            printLog(i);
            inCart[id] =
                await cartController.checkCartItem(id: "${id}00$index");
            if (inCart[id] == true) {
              flag = true;
              break;
            }
  ...
  Future<void> getItemDataBestOrFlash(Item item, String sizes) async {
    List<Item> data = [];
    if (haveData == true) {
      data = await api.apiItemDataBestOrFlash(item,
          sizes: sizes, page: numberPage);
    }
    if (numberPage == 1) {
      allItems.clear();
    }
  ...
  Future<void> getItemDataSpecificInOrder(
      Item item, String sizes, SpecificOrder specificOrder) async {
    allItems = [];
    List<Item>? data =
        await api.apiItemData(item, sizes: sizes, haveSimilar: false, page: 1);
  ...
  Future<void> changeSizesItem(
      String id, String size, Item item, int index) async {
    inCart[id] = await cartController.checkCartItem(id: "${id}00$index");
    sizesItems[id] = size;
    indexItems[id] = index;
